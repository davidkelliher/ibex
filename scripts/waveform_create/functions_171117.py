...
class Waveforms(object):
    ...
    def __init__(self, type, seg_len, offset, accumulation_voltage, voltage_array, ramp_periods, amp_gain, awg_gain, max_bits):
    ...
    def create_accumulation_waveforms(self, seg, rods):
        ...
        if rods == 13:
            for i in range(self.Seg_len):
                accumulation_voltage.append(self.Accumulation_voltage * np.sin(i * 2 * np.pi / self.Seg_len) + self.Offset)
        else:
            for i in range(self.Seg_len):
                accumulation_voltage.append(-1*self.Accumulation_voltage * np.sin(i * 2 * np.pi / self.Seg_len) + self.Offset)


        # convert amplifier voltage to AWG bits
        accumulation_voltage = np.array(accumulation_voltage)
        accumulation_v_bits = (np.array(accumulation_voltage) / self.Amp_gain) * (self.Max_bits / self.Awg_gain)
    ...
    def  create_ramping_waveforms(self, seg, rods):
        for j in range(len(self.Voltage_array)):
            ...
            v_bits = (np.array(voltage) / self.Amp_gain) * (self.Max_bits / self.Awg_gain)
    ...
    def create_final_tune_waveforms(self, seg, rods):
        ...
        for j in range(len(self.Voltage_array)):
            ...
            print "maxbits ", self.Max_bits
            print "accumulation_v_bits ", max(v_bits) - min(v_bits)
    ...
    def create_gate_drop_waveforms(self, seg, rods):
        ...
        for j in range(len(self.Voltage_array)):
            ...
            if rods == 13:
                for i in range(self.Seg_len):
                    voltage.append(self.Voltage_array[j] * np.sin(i * 2 * np.pi / self.Seg_len))
            else:
                for i in range(self.Seg_len):
                    voltage.append(-1 * self.Voltage_array[j] * np.sin(i * 2 * np.pi / self.Seg_len))
