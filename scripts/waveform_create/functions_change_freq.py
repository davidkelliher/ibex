...
class Waveforms(object):
    ...
    def create_accumulation_waveforms(self, seg, rods, phase):
        ...
        if rods == "13":
            voltage = self.Accumulation_voltage * np.sin((pts_a * 2 * np.pi / self.Seg_len)+ phase)/self.Amp_gain[self.Type+rods] + (self.Offset/self.DC_gain)
        ...
        for j in range(len(self.Voltage_array)):
            ...
            for l in range(int(self.Ramp_periods)):
                ...
                for i in range(self.Seg_len):
                    ...
                    if rods == "13":
                        voltage.append(rf_amp*((np.sin(i * 2 * np.pi / self.Seg_len) + phase))/self.Amp_gain[self.Type+rods] + (self.Offset + dc_part)/self.DC_gain)
                    else:
                        voltage.append(-rf_amp*((np.sin(i * 2 * np.pi / self.Seg_len) + phase))/self.Amp_gain[self.Type+rods] + (self.Offset - dc_part) /self.DC_gain)
    ...
    def create_final_tune_waveforms(self, seg, rods, dc_offset, phase):
        ...
        for j in range(len(self.Voltage_array)):
            ...
            if rods == "13":
                for i in range(self.Seg_len):
                    voltage.append(self.Voltage_array[j] * np.sin((i * 2 * np.pi / self.Seg_len) + phase)/self.Amp_gain[self.Type+rods] + (self.Offset + 0.5*self.Offset_diff)/self.DC_gain )
            else:
                for i in range(self.Seg_len):
                    voltage.append(-1 * self.Voltage_array[j] * np.sin((i * 2 * np.pi / self.Seg_len) + phase)/self.Amp_gain[self.Type+rods] + (self.Offset - 0.5*self.Offset_diff)/self.DC_gain)
    ...
    def create_all_waveforms(self, powered):
        FC_phase_13 = -np.radians(7.0)
        FC_phase_24 = -np.radians(7.0)
        if powered ==2:
            rods = "13"
            if self.Type == "CROD":
            ...
            if self.Type == "FC":
                ...
                self.create_ramping_waveforms(2, rods, 0)
                self.create_final_tune_waveforms(3, rods, 0)
                self.create_gate_drop_waveforms(4, rods, 0)
            ...
            if self.Type == "CROD":
                self.create_accumulation_waveforms(1, rods, 0)
                self.create_ramping_waveforms(2, rods, 0)
                self.create_final_tune_waveforms(3, rods, self.Offset, 0)
                self.create_final_tune_waveforms(4, rods, self.Offset, 0)

            if self.Type == "FC":
                self.create_accumulation_waveforms(1, rods, FC_phase_13)
                self.create_ramping_waveforms(2, rods, FC_phase_13)
                self.create_final_tune_waveforms(3, rods, self.Offset, FC_phase_13)
                self.create_final_tune_waveforms(4, rods, self.Offset_drop, FC_phase_13)

            if self.Type == "MCP":
                self.create_accumulation_waveforms(1, rods, 0)
                self.create_ramping_waveforms(2, rods, 0)
                self.create_final_tune_waveforms(3, rods, self.Offset, 0)
                self.create_final_tune_waveforms(4, rods, self.Offset_drop, 0)
            ...
            if self.Type == "FC":
                ...
                self.create_final_tune_waveforms(3, rods, self.Offset, FC_phase_24)
                self.create_final_tune_waveforms(4, rods, self.Offset_drop, FC_phase_24)
