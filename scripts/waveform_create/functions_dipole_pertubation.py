...
class Waveforms(object):
    ...
    def __init__(self, rodtype, seg_len, offset, accumulation_voltage, voltage_array, ramp_periods, amp_gain, awg_connect, awg_gain, max_bits, endcap_offset_drop=0):
        ...
        self.awg_DC_offset = {'C1':0.0028, 'C2':0.0072, 'C3':0.02, 'C4':0.006} #15/1/18
    ...
    #                for i in range(int(pert[2] * self.Seg_len *(1e6/pert[0]))):
                ...
                if rods == "13":
                    for i in range(int(pert[2] * self.Seg_len)):
                        voltage.append((voltage_array[j] * np.sin((i *  2 * np.pi / self.Seg_len)+ phase) /self.Amp_gain[self.Type+rods]) +
                                     (dc_offset/self.DC_gain) + DCawgoff)
                elif rods == "2":
                    for i in range(int(pert[2] * self.Seg_len)):
                        voltage.append((-1 * voltage_array[j] * np.sin((i * 2 * np.pi / self.Seg_len)+ phase) /self.Amp_gain[self.Type+rods]) -
                                       (pert[1] * np.sin((i * 2 * np.pi / (self.Seg_len / (pert[0]/1e6))) + phase) / self.Amp_gain[
                            self.Type + rods]) + (dc_offset/self.DC_gain) + DCawgoff)
    ...
    def create_all_waveforms(self, powered, pertubation):
        ...
        if powered == 2:
            print("this script doesnt do this")
