...
class Waveforms(object):
    ...
    def __init__(self, rodtype, seg_len, offset, accumulation_voltage, voltage_array, ramp_periods, amp_gain, awg_connect, awg_gain, max_bits, endcap_offset_drop=0):
        ...
        self.awg_DC_offset = {'C1':0.0, 'C2':0.0, 'C3':0.0, 'C4':0.0} #account for offset in AWG DC output which varies with channel?
    ...
    def create_accumulation_waveforms(self, seg, rods):
        ...
        if rods == "13":
            for i in range(self.Seg_len):
                voltage.append(self.Accumulation_voltage * np.sin(i * 2 * np.pi / self.Seg_len)/self.Amp_gain[self.Type+rods] + (self.Offset + DCawgoff)/self.DC_gain)
        else:
            for i in range(self.Seg_len):
                voltage.append(-1*self.Accumulation_voltage * np.sin(i * 2 * np.pi / self.Seg_len)/self.Amp_gain[self.Type+rods] + (self.Offset + DCawgoff)/self.DC_gain)
            ...
            for l in range(int(self.Ramp_periods)):
                for i in range(self.Seg_len):
                    if rods == "13":
                         ...
                         * np.sin(i * 2 * np.pi / self.Seg_len))/self.Amp_gain[self.Type+rods] + self.Offset/self.DC_gain)
                    else:
                        voltage.append((((((self.Voltage_array[j]-self.Accumulation_voltage)*((l*self.Seg_len) + i))/(self.Ramp_periods * self.Seg_len)) + self.Accumulation_voltage)
                         * -1 * np.sin(i * 2 * np.pi / self.Seg_len))/self.Amp_gain[self.Type+rods] + self.Offset/self.DC_gain)
    ...
    def create_final_tune_waveforms(self, seg, rods):
        ...
        for j in range(len(self.Voltage_array)):
            ...
            if rods == "13":
                for i in range(self.Seg_len):
                    voltage.append(self.Voltage_array[j] * np.sin(i * 2 * np.pi / self.Seg_len)/self.Amp_gain[self.Type+rods] + (self.Offset + DCawgoff)/self.DC_gain)
            else:
                for i in range(self.Seg_len):
                    voltage.append(-1 * self.Voltage_array[j] * np.sin(i * 2 * np.pi / self.Seg_len)/self.Amp_gain[self.Type+rods] + (self.Offset + + DCawgoff)/self.DC_gain)
    ...
    def create_gate_drop_waveforms(self, seg, rods):
        ...
        for j in range(len(self.Voltage_array)):
            ...
            print "self.Offset_drop ",self.Offset_drop
            if rods == "13":
                for i in range(self.Seg_len):
                    voltage.append(self.Voltage_array[j] * np.sin(i * 2 * np.pi / self.Seg_len)/self.Amp_gain[self.Type+rods] + (self.Offset_drop + DCawgoff)/self.DC_gain)
            else:
                for i in range(self.Seg_len):
                    voltage.append(-1 * self.Voltage_array[j] * np.sin(i * 2 * np.pi / self.Seg_len)/self.Amp_gain[self.Type+rods] + (self.Offset_drop + DCawgoff)/self.DC_gain)
