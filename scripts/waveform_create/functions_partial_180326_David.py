...
class Waveforms(object):
    ...
    def create_fixed_waveforms(self, seg, rods, rf_amp, dc_offset, file_index=None):
        ...
        if rods == "13":
            voltage = rf_amp * np.sin(pts_a * 2 * np.pi / self.Seg_len)/self.Amp_gain[self.Type+rods] + (dc_offset/self.DC_gain)
    ...
    def  create_ramping_waveforms(self, seg, rods, rf_amp_initial_a, rf_amp_final_a, dc_offset):
        ...
        print rf_amp_final_a == rf_amp_initial_a
        print rf_amp_final_a, rf_amp_initial_a
        ...
        for j in range(len(rf_amp_final_a)):
            ...
            for l in range(int(self.Ramp_periods)):
                ...
                for i in range(self.Seg_len):
                    ...
                    if rods == "13":
                        voltage.append(rf_amp*(np.sin(i * 2 * np.pi / self.Seg_len))/self.Amp_gain[self.Type+rods] + (dc_offset + dc_part)/self.DC_gain)
                    else:
                        voltage.append(-rf_amp*(np.sin(i * 2 * np.pi / self.Seg_len))/self.Amp_gain[self.Type+rods] + (dc_offset - dc_part) /self.DC_gain)
    ...
    def create_all_waveforms(self, powered):
        ...
        if powered ==2:
            ...
            rods = "13"
            if self.Type == "CROD":
                self.create_fixed_waveforms(1, rods, self.Accumulation_voltage, self.Offset)
            ...
            if self.Type == "CROD":
                self.create_fixed_waveforms(1, rods, 0, self.Offset)
                self.create_ramping_waveforms(2, rods,  rf_amp_zero_a, rf_amp_zero_a, self.Offset)
                self.create_fixed_waveforms(3, rods, 0, self.Offset, 0)
                self.create_fixed_waveforms(4, rods, 0, self.Offset, 0)
            ...
            if self.Type == "MCP":
                self.create_accumulation_waveforms(1, rods, self.Accumulation_voltage, self.Offset)
                self.create_ramping_waveforms(2, rods, rf_amp_initial_a, self.Voltage_array,  self.Offset)
                self.create_final_tune_waveforms(3, rods, self.Offset)
                self.create_final_tune_waveforms(4, rods, self.Offset_drop)
