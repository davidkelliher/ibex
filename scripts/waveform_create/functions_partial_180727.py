...
class Waveforms(object):
    ...
    def __init__(self, rodtype, seg_len, offset, accumulation_voltage, voltage_array, ramp_periods, amp_gain, awg_connect, awg_gain, max_bits, endcap_offset_drop=0):
        ...
        self.awg_DC_offset = {'C1':0.0025, 'C2':0.007, 'C3':0.012, 'C4':0.005} #15/1/18
    ...
    def create_accumulation_waveforms(self, seg, rods):
        ...
        if rods == "13":
            for i in range(self.Seg_len):
                voltage.append(self.Accumulation_voltage * np.sin(i * 2 * np.pi / self.Seg_len)/self.Amp_gain[self.Type+rods] + (self.Offset/self.DC_gain) + DCawgoff)
        else:
            for i in range(self.Seg_len):
                voltage.append(-1*self.Accumulation_voltage * np.sin(i * 2 * np.pi / self.Seg_len)/self.Amp_gain[self.Type+rods] + (self.Offset/self.DC_gain) + DCawgoff)
    ...
    def create_accumulation_waveforms_square(self, seg, rods):
        ...
        if rods == "13":
            time,v1 = square_wave(offset = self.Offset/self.DC_gain,amplitude = self.Accumulation_voltage/self.Amp_gain[self.Type+rods]) 
            for v in v1:
                voltage.append(v)
        else:
            time,v1 = square_wave(offset = -self.Offset/self.DC_gain,amplitude = self.Accumulation_voltage/self.Amp_gain[self.Type+rods]) 
            ...
            for l in range(int(self.Ramp_periods)):
                for i in range(self.Seg_len):
                    if rods == "13":
                         ...
                         * np.sin(i * 2 * np.pi / self.Seg_len))/self.Amp_gain[self.Type+rods] + self.Offset/self.DC_gain + DCawgoff)
                    else:
                        voltage.append((((((self.Voltage_array[j]-self.Accumulation_voltage)*((l*self.Seg_len) + i))/(self.Ramp_periods * self.Seg_len)) + self.Accumulation_voltage)
                         * -1 * np.sin(i * 2 * np.pi / self.Seg_len))/self.Amp_gain[self.Type+rods] + self.Offset/self.DC_gain + DCawgoff)
    ...
    def  create_ramping_waveforms_square(self, seg, rods):
        ...
        for j in range(len(self.Voltage_array)):
            ...
            for l in range(int(self.Ramp_periods)):
                slope = (self.Voltage_array[j]-self.Accumulation_voltage)/self.Ramp_periods
                amplitude = ((slope*l))+self.Accumulation_voltage
                if rods == "13":
                    time,v1 = square_wave(offset = self.Offset/self.DC_gain,amplitude = amplitude/self.Amp_gain[self.Type+rods])
                    for v in v1:
                        voltage.append(v)
                else:
                    time,v1 = square_wave(offset = -self.Offset/self.DC_gain,amplitude = amplitude/self.Amp_gain[self.Type+rods]) 
    ...
    def create_final_tune_waveforms(self, seg, rods):
        ...
        for j in range(len(self.Voltage_array)):
            ...
            if rods == "13":
                for i in range(self.Seg_len):
                    voltage.append(self.Voltage_array[j] * np.sin(i * 2 * np.pi / self.Seg_len)/self.Amp_gain[self.Type+rods] + (self.Offset/self.DC_gain) + DCawgoff)
            else:
                for i in range(self.Seg_len):
                    voltage.append(-1 * self.Voltage_array[j] * np.sin(i * 2 * np.pi / self.Seg_len)/self.Amp_gain[self.Type+rods] + (self.Offset/self.DC_gain) + DCawgoff)
    ...
    def create_final_tune_waveforms_square(self, seg, rods):
        ...
        for j in range(len(self.Voltage_array)):
            ...
            if rods == "13":
                time,v1 = square_wave(offset = self.Offset/self.DC_gain,amplitude = self.Voltage_array[j]/self.Amp_gain[self.Type+rods])
                for v in v1:
                    voltage.append(v)
            else:
                time,v1 = square_wave(offset = -self.Offset/self.DC_gain,amplitude = self.Voltage_array[j]/self.Amp_gain[self.Type+rods])
    ...
    def create_gate_drop_waveforms_square(self, seg, rods):
        ...
        for j in range(len(self.Voltage_array)):
            ...
            if rods == "13":
                time,v1 = square_wave(offset = self.Offset_drop/self.DC_gain,amplitude = self.Voltage_array[j]/self.Amp_gain[self.Type+rods])
                for v in v1:
                    voltage.append(v)
            else:
                time,v1 = square_wave(offset = -self.Offset_drop/self.DC_gain,amplitude = self.Voltage_array[j]/self.Amp_gain[self.Type+rods])
    ...
    def create_gate_drop_waveforms(self, seg, rods):
        ...
        for j in range(len(self.Voltage_array)):
            ...
            if rods == "13":
                for i in range(self.Seg_len):
                    voltage.append(self.Voltage_array[j] * np.sin(i * 2 * np.pi / self.Seg_len)/self.Amp_gain[self.Type+rods] + (self.Offset_drop/self.DC_gain) + DCawgoff)
            else:
                for i in range(self.Seg_len):
                    voltage.append(-1 * self.Voltage_array[j] * np.sin(i * 2 * np.pi / self.Seg_len)/self.Amp_gain[self.Type+rods] + (self.Offset_drop/self.DC_gain) + DCawgoff)
