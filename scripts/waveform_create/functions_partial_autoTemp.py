...
class Waveforms(object):
    ...
    def __init__(self, rodtype, seg_len, offset, accumulation_voltage, voltage_array, ramp_periods, amp_gain, awg_connect, awg_gain, max_bits, endcap_array):

        self.Type = rodtype
        self.Seg_len = seg_len
        self.Offset = offset
        self.Offset_drop = endcap_array
    ...
    def  create_ramping_waveforms(self, seg, rods, phase):
        ...
        for j in range(len(self.Offset_drop)):
            ...
            for l in range(int(self.Ramp_periods)):
                for i in range(self.Seg_len):
                    if rods == "13":
                        voltage.append((((((self.Voltage_array-self.Accumulation_voltage)*((l*self.Seg_len) + i))/(self.Ramp_periods * self.Seg_len)) + self.Accumulation_voltage)
    ...
    def create_final_tune_waveforms(self, seg, rods, phase):
        ...
        for j in range(len(self.Offset_drop)):
            ...
            if rods == "13":
                for i in range(self.Seg_len):
                    voltage.append(self.Voltage_array * np.sin((i * 2 * np.pi / self.Seg_len) + phase)/self.Amp_gain[self.Type+rods] + (self.Offset/self.DC_gain) + DCawgoff)
            else:
                for i in range(self.Seg_len):
                    voltage.append(-1 * self.Voltage_array * np.sin((i * 2 * np.pi / self.Seg_len) + phase)/self.Amp_gain[self.Type+rods] + (self.Offset/self.DC_gain) + DCawgoff)
    ...
    def create_gate_drop_waveforms(self, seg, rods, phase):
        ...
        for j in range(len(self.Offset_drop)):
            ...
            print "seg, self.Offset_drop ",seg, self.Offset_drop[j]
            if rods == "13":
                for i in range(self.Seg_len):
                    voltage.append(self.Voltage_array * np.sin((i * 2 * np.pi / self.Seg_len) + phase)/self.Amp_gain[self.Type+rods] + (self.Offset_drop[j]/self.DC_gain) + DCawgoff)
            else:
                for i in range(self.Seg_len):
                    voltage.append(-1 * self.Voltage_array * np.sin((i * 2 * np.pi / self.Seg_len) + phase)/self.Amp_gain[self.Type+rods] + (self.Offset_drop[j]/self.DC_gain) + DCawgoff)
