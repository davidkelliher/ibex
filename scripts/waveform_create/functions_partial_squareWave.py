...
class Waveforms(object):
    ...
    def __init__(self, rodtype, seg_len, offset, accumulation_voltage, voltage_array, ramp_periods, amp_gain, awg_connect, awg_gain, max_bits, endcap_offset_drop=0):
        ...
        self.awg_DC_offset = {'C1':0.0028, 'C2':0.0072, 'C3':0.012, 'C4':0.005} #15/1/18
    ...
    def create_accumulation_waveforms(self, seg, rods, phase):
        ...
        if rods == "13":
            arr = np.concatenate((np.linspace(0, 0, (self.Seg_len / 8) + fraction_moved), np.linspace(1, 1, self.Seg_len / 4),
                                  np.linspace(0, 0, self.Seg_len / 4), np.linspace(-1, -1, self.Seg_len / 4),
                                  np.linspace(0, 0, (self.Seg_len / 8) - fraction_moved)))
            voltage = (self.Accumulation_voltage * arr /self.Amp_gain[self.Type+rods]) + (self.Offset/self.DC_gain) + DCawgoff
    ...
    def  create_ramping_waveforms(self, seg, rods, phase):
        ...
        for j in range(len(self.Voltage_array)):
            ...
            for l in range(int(self.Ramp_periods)):
                ...
                if rods == "13":
                    arr = np.concatenate((np.linspace(0, 0, (self.Seg_len / 8) + fraction_moved), np.linspace(1, 1, self.Seg_len / 4),
                                            np.linspace(0, 0, self.Seg_len / 4),
                                            np.linspace(-1, -1, self.Seg_len / 4),
                                            np.linspace(0, 0, (self.Seg_len / 8) - fraction_moved)))
                    voltage = np.concatenate((voltage, (((((self.Voltage_array[j]-self.Accumulation_voltage)*l)/(self.Ramp_periods)) + self.Accumulation_voltage)
                                * arr / self.Amp_gain[self.Type + rods]) + (
                        self.Offset / self.DC_gain) + DCawgoff))
                else:
                    arr = np.concatenate((np.linspace(0, 0, (self.Seg_len / 8) + fraction_moved), np.linspace(-1, -1, self.Seg_len / 4),
                                            np.linspace(0, 0, self.Seg_len / 4),
                                            np.linspace(1, 1, self.Seg_len / 4),
        ...
        for j in range(len(self.Voltage_array)):
            ...
            if rods == "13":
                arr = np.concatenate((np.linspace(0, 0, (self.Seg_len / 8) + fraction_moved), np.linspace(1, 1, self.Seg_len / 4),
                                      np.linspace(0, 0, self.Seg_len / 4), np.linspace(-1, -1, self.Seg_len / 4),
                                      np.linspace(0, 0, (self.Seg_len / 8)- fraction_moved)))
                voltage = (self.Voltage_array[j] * arr / self.Amp_gain[self.Type + rods]) + (
                self.Offset / self.DC_gain) + DCawgoff
        ...
        for j in range(len(self.Voltage_array)):
            ...
            if rods == "13":
                arr = np.concatenate((np.linspace(0, 0, (self.Seg_len / 8) + fraction_moved), np.linspace(1, 1, self.Seg_len / 4),
                                      ...
                                      np.linspace(0, 0, (self.Seg_len / 8) - fraction_moved)))
                voltage = (self.Voltage_array[j] * arr / self.Amp_gain[self.Type + rods]) + (
                self.Offset_drop / self.DC_gain) + DCawgoff
    ...
    def create_all_waveforms(self, powered):
        # for square waveforms only small phase shifts < 90 degrees can be implemented
        # enter phase shift in degrees - may not be mega accurate as have limited points to play with
        FC_phase_13 = -7.0
        FC_phase_24 = -7.0
        if powered ==2:
            rods = "13"
