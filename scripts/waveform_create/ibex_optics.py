...
class optics(object):
	...
	def construct_waveform(self):
		...
		elif self.lat_type == "step":
			self.ta = np.linspace(0,1,self.npts)
			arr = np.concatenate((np.linspace(0, 0, self.npts/8) , np.linspace(1, 1, self.npts/4), np.linspace(0, 0, self.npts/4), np.linspace(-1, -1, self.npts/4), np.linspace(0, 0, self.npts/8)))
			self.va = self.v0 * arr
